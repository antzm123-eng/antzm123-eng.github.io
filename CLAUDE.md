...
**2026-09-06 리뉴얼 L안 이식 완료.** 사이트가 L안 구조다.
...
1. 🔴 **검사 도구 2개가 새 구조를 못 본다** — 사진을 JS 가 만들어서 `index.html`
   본문에 안 보인다. `check_images` 922→**50개**만, `check_covers` 는 `999%` 헛값.
   `DATA`+`EXT`+`shots` 로 경로를 만들어 검사하도록 고칠 것 (WORKLOG)

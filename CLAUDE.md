...
| 썸네일 최대 변 | 700px (세로 4장은 750px — 기존 예외) |
...
원본 277장 + AVIF 264장 **전부** 저작권 정보 포함(100%). JPEG/PNG 를 재인코딩했다면
저작권을 다시 넣어야 한다(AVIF 는 불필요).
...
- 작업 카드 = **대표 사진 1장 크게** + 장수 배지 + 라이트박스. 커버 프레임은 이미지 비율에 따라
  `data-ratio="wide|square|tall"` (3:2/1:1/4:5). 표시 폭 390화면=277px · 800=623 · 1440=336
...
3. **커버 선명도** — 641~800px 구간은 커버가 최대 623px 로 표시되는데 썸네일은 700px 라
   2배 화면 해상도가 **56%**(다른 폭은 93~100%). **세로 사진은 폭이 500px 뿐이라 1배에서도
   부족하다.** 해결안: 그 구간 커버 폭 제한, 또는 썸네일을 1280px 로 재생성(용량 증가 주의)
...
5. (보류) **퇴사 결정 후** — 회사 작업물 업로드 + 사명 공개 여부 재검토

...
1. **회사 업무 경력을 글로 추가** ← 현재 진행 중. 아래 "경력 섹션" 참고
2. 미업로드 작업물 추가 (사용자가 폴더 준비 중)
3. 전체 검수
4. 커밋 + 푸시
5. GitHub Pages 공개 (사용자 결정 사항)

## 경력 섹션 (설계 확정, 미구현)

사진 없이 **글로만** 회사 업무를 기록할 영역. `#work` 다음, `#contact` 앞에 새 `<section id="career">`
로 추가하기로 함. Work 그리드에 섞지 않음 — 사진 없는 카드가 그리드에 끼면 휑해 보이기 때문.
...
스타일은 기존 `.stat-list` / `.section-head` 계열 재사용. 변수: `--bg-card`, `--line`,
`--ink`, `--ink-dim`, `--orange`. 섹션 여백은 `padding:130px 6vw`(모바일 `90px 6vw`).
`nav` 메뉴에 `CAREER` 링크 추가 필요.

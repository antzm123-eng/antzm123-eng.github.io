...
⚠️ **크기만 재고 끝내지 말 것** — 눌러보고 눈으로도 볼 것(숫자는 맞는데 누웠던 적).
⚠️ **창이 숨겨져 있으면 검사가 거짓말을 한다** — 전환·`lazy` 가 멈추고 화면이 새까맣다.
코드 탓 아님. 콘솔 말고 `performance` 로 볼 것. `scrollTo` 도 안 먹으니
`scrollBehavior='auto'` 부터(아래는 `.main` 에 음수 `margin-top`).
...
8. **파일이 있고 크기가 맞아도 안 열릴 수 있다** → `check_images.py` 로 실제 디코딩.
   홀수 픽셀이면 AVIF 가 깨진다 — `to_avif.swift` 가 짝수로 잘라 방지.
...
작업·갤러리 70개(사진 37·디자인 33) / 사진 324장. **L안 구조**(이식 2026-09-06).

- 구조 `첫화면 → 장 3개 → 열람실 → 경력 → 활동 → 연락` · 사이드바 296px(≤1000px 숨김)
- **열람실이 핵심** — 목록 70줄 + 검색 + 분류 + 뷰어(띠·라이트박스). 사진 마크업은
  **JS `DATA`/`EXT`/`FULLEXT` 가 생성**(`key_N`). 예전 `work-card`·`data-ratio` 없음
- 장(章) 사진 12장 = `<button class="shot" data-jump="키">` — 누르면 그 작업이 열린다.
  휴대폰은 `order:-1` 로 **사진이 제목보다 먼저**(첫 화면에 4장)
- **목차는 5칸이 한계**(6칸=411px 요구 → 375px 에서 잘림). `경력 3` 이 두 칸을 가리킨다
...
- ⚠️ 활동란 `55건·276장·11개 단체` 는 **`DATA` 에서 센 값**(2023.05 이후 집회·팀 맥락).
  작업을 더 넣으면 **같이 고칠 것**
- 검사 도구 3종은 **`DATA` 표를 읽는다**(2026-09-07) — 구조를 또 바꾸면
...
포스터 3장 화질(44%)은 원본 소실로 확정 · "히어로 배경·모션 금지" 폐기(WORKLOG).
...
**고치기 전에 DECISIONS "경력 섹션 서술 규칙" 을 읽을 것**(기밀·계약 소지). 요약:
사명·공간명·지역 비공개 · 순서 `공간기획→마케팅→교육(보조)` 고정 · "운영"은 범위 한정 ·
**검증 불가능한 수치 금지**(활동란도).

...
- 카드 = 대표 사진 + 장수 배지 + 라이트박스 · `data-ratio="wide|square|tall"`
  (3:2/1:1/4:5) · 커버 21장은 아래 7% 잘라 재생성, **포스터 3장만 예전**(44%)
- 섹션 순서 `about → work → career → contact` · 전환은 `--ease` 하나, `:active` 있음
- 콘솔 오류 0 · alt 100% · 가로 스크롤 0 · 날짜 역순 검증 · 대비 AA(미달 1건 ↓)
- **AI 배경 3개**(유튜브 썸네일)는 카드 설명·경력란에 구분 표기(추가 시 동일) ·
  파나소닉 LX2 5묶음은 **의도적 무보정**이라 설명에 명시
- **리뉴얼은 L안**(`_sim/renewal-l.html`, gitignore) — K안 뼈대 + 확정 디자인 +
  검수 10건 해결 · 첫 화면 대표작 창 제거(글칸 `max-width:720px`).
  **`index.html` 은 아직 그대로**

## 남은 일

1. 🟡 **리뉴얼 L안을 `index.html` 에 옮기기.** ⚠️ 중립화·N/A 통일은 **시뮬에만** ·
   첫 화면 대표작 6장은 **2026-09-06 폐기**(피드백)
2. 🔴 `noindex` 로 차단 중. **퇴사 시점에 공개** — `index, follow` +
   Search Console 등록
3. 🟢 **디자인 검수 3건**(2026-09-06 실측, WORKLOG) — `.role-tab .num` 대비 3.73 ·
   모바일 누르는 영역 7개 <44px(햄버거 34×26) · 글씨 13종
4. (보류) **퇴사 후** — 회사 작업물 업로드 · 사명 공개 재검토
...
위계는 색이 아니라 **크기·굵기** — 회색을 늘리지 말 것. 제목 4건(`bureuneun1·2`·
`bureusim`·`bureusim_info`)은 **중립화 제외.** 검수 `_sim/K안-검수와-초기디자인-기획.md`

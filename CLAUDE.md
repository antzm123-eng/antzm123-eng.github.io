...
- `DATA.why` 가 있으면 뷰어에 **「왜 이렇게 했나」** 상자(지금 5건)
- `meta` 셋째 칸은 **맥락** — `외부 단체 요청` 22 / `소속 팀 N/A` 43 / `개인 작업` 8.
  목록 `data-q`(검색 색인, 소문자)와 **함께** 고칠 것
- **대표작 8칸**(`#picks`) — 정사각. **윗줄 4칸 사진 · 아랫줄 4칸 디자인**(줄마다 통일,
  각 줄 안에서는 최신순). 디자인은 `data-kind="design"` → `contain`.
  목차 `03 작업` 이 여기를 가리킨다
- 장(章) 사진 12장 = `<button class="shot" data-jump="키">`. 휴대폰은 `order:-1` 로
  **사진이 제목보다 먼저**(첫 화면에 4장)
- **후기 3건**(`#voices`, 활동과 연락 사이) — 전부 **익명**, 사용자가 받아온 실제 말을
  다듬은 것. **문구를 지어내지 말 것** · 더 받으면 `.voice` 복사. 목차 칸 없음(5칸 한계)
...
- ⚠️ 활동란 `64건·296장` 은 **맥락 라벨에서 센 값**(외부+팀, 2023.05 이후).

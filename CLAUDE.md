...
- 장(章) 사진 12장 = `.shot[data-jump]`. 휴대폰은 `order:-1` 로 **사진이 제목보다 먼저**
- **후기 3건**(`#voices`) — 전부 **익명**. **지어내지 말 것** · 더 받으면 `.voice` 복사
- **열람실 숫자는 JS 가 다시 센다**(눈금·막대·칩·`.yr .n`·머리말). ⚠️ 안 누르면
  안 돈다 → 작업을 넣으면 **HTML 초기 숫자도 손으로 고칠 것**
- **목차는 5칸이 한계**(WORKLOG). `경력 3` 이 경력·활동을 가리킨다
- 지켜야 할 선 — 콘솔 오류 0 · 대비 AA · 글씨 6종 · 가로 스크롤 0 · 누르는 영역 44px
  (≤1000px). 푸터 문장 속 링크만 28px(의도)
- **직접 만들지 않은 소재는 반드시 밝힌다** — AI 배경 3개 · 콜라주 1건
  (`analog_lent`). 작업 설명과 경력란 주석 **둘 다**. 추가 시 동일 ·
  LX2 7묶음은 **의도적 무보정**이라 설명에 명시
- 장수 라벨은 **「이미지」**(첫화면·머리말·활동란 3곳) — 사진만이 아님
- ⚠️ **제목·설명은 진짜 것**(`_sim/` 중립화본 금지) · 설명은 **사진을 열어 실제
  찍힌 것만** · 길이는 **1장 40자대 / 4장+ 95~101자**(상한 101)
- ⚠️ 활동란 `64건·296장` 은 **맥락 라벨에서 센 값**(외부+팀, 2023.05 이후) · 단체는
  **13곳 이름 나열**. 작업을 더 넣으면 **같이 고칠 것**(DECISIONS)

## 남은 일

1. 🔴 `noindex` 차단 중. **퇴사 시점에 공개** — `index, follow` + `sitemap.xml`(없음)
   + Search Console 등록
